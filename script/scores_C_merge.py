...
def main():
  ...
  for group in ['dev','eval']:
    ...
    for tmodel_id in tmodels_ids:
      ...
      for split_id in range(0,N_MAX_SPLITS):
        ...
        if split_id == 0 and not os.path.exists(split_path):
          ...
          break
        C_tmp = torch.core.array.load(split_path)
        if tm_started == False:
          C_tm = C_tmp
          tm_started = True
        else:
          C_tm = torch.core.array.cat((C_tm, C_tmp), 1)
      if full_started == False:
        C_full = C_tm
        full_started = True
      else:
        C_full = torch.core.array.cat((C_full, C_tm), 0)
    ...
    for model_id in models_ids:
      ...
      # Saves to files
      torch.io.Array(C_extracted).save(os.path.join(config.zt_norm_C_dir, group, str(model_id) + ".hdf5"))
