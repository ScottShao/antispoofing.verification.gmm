...
def getModel(model_id, models_dir):
  ...
  return torch.core.array.load(model_path)
...
def main():
  ...
  for model_id in models_ids:
    ...
    probe_files = db.objects(directory=config.features_dir, extension=config.features_ext, groups=args.group, protocol=config.protocol, purposes="probe", model_ids=(model_id,))
    ...
    import lgbphs
    lgbphs.lgbphs_scores_A((model_id,), config.models_dir, probe_files, db, 
