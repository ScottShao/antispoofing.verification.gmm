    ...
    for tmodel_id in tnorm_models_ids:
      ...
      for split_id in range(0,N_MAX_SPLITS):
        ...
        if split_id == 0:
          ...
          break
        D_tmp = torch.core.array.load(split_path_D)
        D_s_tmp = torch.core.array.load(split_path_Ds)
        if not tm_started:
          D_tm = D_tmp
          D_s_tm = D_s_tmp
          tm_started = True
        else:
          D_tm = torch.core.array.cat((D_tm, D_tmp), 1)
          D_s_tm = torch.core.array.cat((D_s_tm, D_s_tmp), 1)
      if not started:
        D = D_tm
        D_s = D_s_tm
        started = True
      else:
        D = torch.core.array.cat((D, D_tm), 0)
        D_s = torch.core.array.cat((D_s, D_s_tm), 0)
    # Saves to files
    torch.io.Array(D).save(os.path.join(config.zt_norm_D_dir, group, "D.hdf5"))
    torch.io.Array(D_s).save(os.path.join(config.zt_norm_D_sameValue_dir, group, "D_sameValue.hdf5"))
